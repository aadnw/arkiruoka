...
def favorite(recipe_id, user_id):
    sql = """UPDATE favorites SET visible=1
...
def favorites(user_id):
    sql = """SELECT R.id, R.name FROM recipes R, favorites F, users U
            WHERE R.id=F.recipe_id AND F.user_id=U.id AND R.visible=1 AND F.visible=1
            ORDER BY R.name"""
    return db.session.execute(text(sql), {"user_id":user_id}).fetchall()

def is_favorite(recipe_id, user_id):
    sql = "SELECT visible FROM favorites WHERE recipe_id=:recipe_id AND user_id=:user_id"
    result = db.session.execute(text(sql), {"recipe_id":recipe_id, "user_id":user_id}).fetchone()
    if result != None:
        return result[0]
    else:
        return 0
...
def get_top_recipes():
    sql = """SELECT R.name, R.id, W.recipe_id, AVG(W.stars) FROM recipes R, reviews W
            WHERE R.id=W.recipe_id AND R.visible=1
